...
func (r *userRepo) CreateUser(ctx context.Context, u *biz.User) (*biz.User, error) {
	...
	if res.Error != nil {
		return nil, status.Errorf(codes.Internal, "%v", res.Error)
	}
	return &biz.User{
		ID:       user.ID,
		Mobile:   user.Mobile,
...
// List .
func (r *userRepo) List(ctx context.Context, pageNum, pageSize int) ([]*biz.User, int, error) {
	var users []User
...
func (r *userRepo) UpdateUser(ctx context.Context, user *biz.User) (bool, error) {
	...
	if res.Error != nil {
		return false, status.Errorf(codes.Internal, "更新用户失败: %v", res.Error)
	}
...
// UserById .
func (r *userRepo) UserById(ctx context.Context, Id int64) (*biz.User, error) {
	var user User
